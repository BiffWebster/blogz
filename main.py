...
def login():
    if request.method == "POST":
        ...
        if member and password == member.password:
            ...
            if not member:
                return render_template('home.html', registered_username=username,
                                       registered_username_error="Username does not exist")

            else:
                return render_template('home.html', registered_username=username,
                                       stored_password_error="Invalid Password")
    else:
...
def create_username():
    if request.method == "POST":
        ...
        if already_exists:
            return render_template('createusername.html', username=username, email=email,
                                   username_error="already_exists", password_error=p_test, verify_error=pv_test,
                                   email_error=e_test)
        elif u_test != "" or p_test != "" or pv_test != "" or e_test != "":
            return render_template('createusername.html', username=username, email=email, username_error=u_test,
                                   password_error=p_test, verify_error=pv_test, email_error=e_test)
        else:
...
def create_blog():
    if request.method == 'POST':
        ...
        if blog_name_validate(blog_name) or blog_body_validate(blog_body) != "":
            return render_template('newposts.html', title="Add a blog", title_error=blog_name_validate(blog_name),
                                   content_error=blog_body_validate(blog_body), old_name=blog_name, old_post=blog_body)
        else:
